...
export default function Menu() {
	...
	if (location.pathname === "/sklep") {
		...
		routes = [
			...
			{ name: "POŁĄCZENI W KRYZYSIE", to: "/polaczeni_w_kryzysie" },
		];
	} else {
		routes = [{ name: "STRONA GŁÓWNA", to: "/" }];
	}
	...
	const renderRoute = (route: Route) => {
		...
		if (route.to) {
			return (
				<ScrollLink to={route.to} onClick={handleMenuItemClick}>
					...
					<div ref={ref} className="lg:hidden">
						...
						<AnimatePresence>
							{isOpen && (
								...
								>
									<ul
										className={`grid gap-2 containerMenu ${isOpen ? "active" : ""}`}
									>
										{routes.length > 0 ? (
